...
import { MessageEvents, ReactionEvents } from "./events"
...
export class Messages {
  ...
  private channelListeners = new WeakMap<
  ...
  constructor(
    ...
    channel: RealtimeChannelPromise,
    chatApi: ChatApi
  ) {
  ...
  async send(text: string): Promise<Message> {
    return this.makeMessageApiCallAndWaitForRealtimeResult(
  ...
  async addReaction(messageId: string, reactionType: string) {
    return this.makeReactionApiCallAndWaitForRealtimeResult(
      ReactionEvents.added,
      async () => {
        const { id } = await this.chatApi.addMessageReaction(
          this.conversationId,
          messageId,
          reactionType
        )
  ...
  private async makeMessageApiCallAndWaitForRealtimeResult(
    event: MessageEvents,
  ...
  private async makeReactionApiCallAndWaitForRealtimeResult(
